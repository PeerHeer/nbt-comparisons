...
# Return the type ID.
execute store result score #typeid.type_id typeid.var run data get storage typeid:cache Entry[0].ID
...
data modify storage typeid:result TypeStr set from storage typeid:cache Entry[0].Str

# Remove the appended entry from the cache list.
data remove storage typeid:cache Entry[0].List[1]