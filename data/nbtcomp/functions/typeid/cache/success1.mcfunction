...
# Return the type ID.
execute store result score #nbtcomp.typeid.type_id nbtcomp.var run data get storage nbtcomp:typeid.cache Entry[1].ID
...
data modify storage nbtcomp:typeid.result TypeStr set from storage nbtcomp:typeid.cache Entry[1].Str

# Remove the appended entry from the cache list.
data remove storage nbtcomp:typeid.cache Entry[1].List[1]